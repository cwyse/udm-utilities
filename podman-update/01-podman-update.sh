...
while [ ! -f $CONMON_DL ]; do
  curl -fsSLo $CONMON_DL https://github.com/cwyse/udm-utilities/blob/$PODMAN_VERSION/podman-update/bin/conmon-$CONMON_VERSION?raw=true
  sleep 1
done
chmod +x $CONMON_DL
...
if [ ! -f $PODMAN_DL ]; then
  curl -fsSLo $PODMAN_DL https://github.com/cwyse/udm-utilities/blob/$PODMAN_VERSION/podman-update/bin/podman-$PODMAN_VERSION?raw=true
fi
chmod +x $PODMAN_DL
...
if [ ! -f $RUNC_DL ]; then
  curl -fsSLo $RUNC_DL https://github.com/cwyse/udm-utilities/blob/$PODMAN_VERSION/podman-update/bin/runc-$RUNC_VERSION?raw=true
fi
chmod +x $RUNC_DL
...
if [ ! -f $SECCOMP ]; then
  ...
  curl -fsSLo $SECCOMP https://github.com/cwyse/udm-utilities/blob/$PODMAN_VERSION/podman-update/bin/seccomp.json?raw=true
fi
